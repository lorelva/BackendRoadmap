...
func main() {
	...
	if result == -1 {
		fmt.Println("No se encontró el numero")
...
func busquedaBinaria(array []int, numeroEncontrar int) int {
	...
	for i := 0; i <= len(array)-1; i++ {
		...
		if valor == numeroEncontrar {
			fmt.Printf("Número %d encontrado\n", numeroEncontrar)
			//Cuando se encuentra el numero se retorna y se sale del for y de la función
			return posicion
		}
		if valor < numeroEncontrar {
			inicio = posicion + 1
